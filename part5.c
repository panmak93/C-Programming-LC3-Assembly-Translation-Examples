...
unsigned short operation(char operator[]) {
	...
	else if(strncmp(operator, "br", 2) ==0)instruction = 0x0000;
...
unsigned short cond(char condition[]){
	int index;
	index = 0;
	unsigned short shift;
	shift = 0x0000;
...
int main(int argc,char *argv[]) { 
	if ( argc != 4 ) {
		...
		char str[100], *operator, *operand, *operand1, *operand2, *dest;
		...
		do{
			...
			else if(strncmp(operator, "br", 2) ==0){
				char condition[3];
				memcpy(condition, &operator[2], 3);	
				shift = cond(condition);
				instruction = instruction | shift;
				operand = strtok (NULL, delimiter);
				shift = compare(argv[2], operand, add);
				instruction = instruction | shift;
			}
			else{
